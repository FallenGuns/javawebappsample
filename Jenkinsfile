...
node {
  withEnv(['AZURE_SUBSCRIPTION_ID=cb0f52cf-cd9e-4a13-b557-7dbf7ba9d3a0',
    ...
    stage('deploy') {
      ...
      // upload package
      sh "az webapp deploy --resource-group jenkins-get-started-rg --name hanqingdingapp --src-path target/calculator-1.0.war --type war"
      // log out
      sh 'az logout'
    }
  }
